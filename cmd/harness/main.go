...
func main() {
	...
	mcp.AddTool(server, &mcp.Tool{Name: "step_scanline", Description: "Advance until the TV scanline increments once (stops at the next scanline, or scanline 0 of the next frame). Returns the CPU cycles EXECUTED across that scanline (not the line's 76 machine cycles: WSYNC stall time is excluded, so a kernel that does 8 cycles of work and then waits reports 8, not 76) and beam coords — for inspecting kernel state line by line. Do NOT read the remainder as headroom; for how much of a line is actually spent use assert_line_budget (one run) or prove_line_budget (all paths)."}, handleStepScanline)
	...
	mcp.AddTool(server, &mcp.Tool{Name: "read_audio", Description: "Read the current TIA audio state for both channels: control (AUDC, waveform), freq (AUDF, divider), volume (AUDV), AND note0/note1 = the pitch each channel is actually sounding as {note, cents} — the nearest 12-TET note name plus the deviation in cents. That pitch conversion exists nowhere else in this tool surface (read_audio_trace returns raw registers only), so this is the tool for 'is the jingle in tune', 'what note is that', 'is this engine hum an F#'. Lets you verify sound numerically — read_tia/read_row only cover video. A silent channel (volume 0) reports no note."}, handleReadAudio)
	...
	mcp.AddTool(server, &mcp.Tool{Name: "breakif", Description: "Run up to max_frames, halting when the beam REACHES (until_scanline, until_clock) — the first instruction boundary at or past that position, in read_row/beamtrace clock coordinates (HBLANK -68..-1, visible 0..159). Not an exact match: the CPU is only observed every 3 colour clocks and a WSYNC kernel only a few times per line, so requiring equality made the call run to max_frames and return halted=false for most positions, with no error. A position already passed in the current frame is caught on the next frame. Out-of-range clocks are rejected."}, handleBreakIf)
	mcp.AddTool(server, &mcp.Tool{Name: "assert_line_budget", Description: "Run up to max_frames and halt the moment a logical line (the interval between WSYNC strobes) overruns its CPU-cycle budget and eats extra scanlines — the failure mode that silently rolls the screen. budget defaults to 76 (one scanline); raise it for multi-line kernels. Returns over=true with at_scanline (the overrunning line's start) and line_cycles = the scanlines that logical line ate x 76. It is a QUANTISED figure, always a multiple of 76, not a measured cycle count: it says 'this line spilled into 2 physical scanlines', not 'this line took 152 cycles'. Do not subtract the budget from it and treat the difference as the number of cycles to cut. Observes ONE run (∃) — for an all-paths proof use prove_line_budget. Optional patch=[{symbol|addr,bytes}] applies temporary ROM byte patches to a COPY for this measurement only (e.g. a lightweight positioning table), fresh-boots it, and ALWAYS reloads the original ROM afterwards — replaces the hand-edit→assemble→assert→restore ritual and its forget-to-restore failure mode. pokes=[{addr,value}] seeds RAM after the patched boot."}, handleBudgetGuard)
	...
	registerCeilingTools(server) // visual_ceiling (cmd/harness/tools_ceiling.go)
