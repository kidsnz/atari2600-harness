...
func GenerateSymmetricASM(art []string, water, lily []byte, opts SceneOpts) (string, error) {
	...
	fmt.Fprintf(&b, `; monet_m1 — generated by roms/frogger/gen (pkg/playfield.GenerateSymmetricASM)
...
func GenerateAsymmetricASM(art []string, water []byte, lily byte, opts SceneOpts) (string, error) {
	...
	var b strings.Builder
	fmt.Fprintf(&b, `; asym playfield — generated by pkg/playfield.GenerateAsymmetricASM
...
func GenerateMonetSpriteASM(bg, grp0 []byte, nusiz, colup0, hmp0 byte) (string, error) {
	...
	var b strings.Builder
	fmt.Fprintf(&b, `; Monet water + flowing lily sprite — pkg/playfield.GenerateMonetSpriteASM
...
func GenerateMonetFullASM(bg, grp0, grp1 []byte, nusiz0, nusiz1, colup0, colup1, hmp0 byte) (string, error) {
	...
	var b strings.Builder
	fmt.Fprintf(&b, `; Monet full scene — pkg/playfield.GenerateMonetFullASM
...
// GenerateFroggerASM は遊べる Monet Frogger（M5）。Monet 水面の上で、流れる睡蓮(player0)を
// 足場にカエル(player1)が上下 hop で川を渡る。状態機械で ride/drown/win/lives を処理。
//   - 川レーン: frog が FrogY=80（睡蓮の帯）に居て、葉に乗っていなければ(CXPPMM クリア)溺れて start へ。
//   - 乗っていれば葉と一緒に流れる（HMP1=葉ドリフト）。入力(左右)は上書き、上下は1レーン hop。
//   - FrogY<=16 でゴール（Score++、start へ）。drown で Lives--。
//
// RAM: FrogY=$81, PrevU=$82, PrevD=$83, Lives=$84, Score=$85, OnPad=$86。peek で観測可能。
// bg/grp0 は長さ192（grp0=睡蓮の川レーン scanline 80-87 に配置）。frogGfx は 8 バイト。
func GenerateFroggerASM(bg, grp0 []byte, frogGfx []byte, nusiz0, colup0, colup1, hmp0 byte) (string, error) {
	if len(bg) != 192 || len(grp0) != 192 {
		return "", fmt.Errorf("bg/grp0 must be length 192")
	}
	if len(frogGfx) != 8 {
		return "", fmt.Errorf("frogGfx must be length 8")
	}
	emit := func(name string, vals []byte) string {
		var sb strings.Builder
		sb.WriteString(name + "\n")
	...
	var b strings.Builder
	fmt.Fprintf(&b, `; Monet Frogger (playable) — pkg/playfield.GenerateFroggerASM
        processor 6502
VSYNC   = $00
VBLANK  = $01
WSYNC   = $02
...
`, colup0, colup1, nusiz0, hmp0)
	b.WriteString(emit("FrogGfx", frogGfx))
	b.WriteString("\n")
	b.WriteString(emit("GRP0Tab", grp0))
	b.WriteString("\n")
	b.WriteString(emit("BGTab", bg))
...
func GenerateAsymmetricShimmerASM(art []string, waterCycle []byte, lily byte, opts SceneOpts) (string, error) {
	...
	var b strings.Builder
	fmt.Fprintf(&b, `; asym Monet shimmer — pkg/playfield.GenerateAsymmetricShimmerASM
