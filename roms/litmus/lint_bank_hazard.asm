; lint_bank_hazard — FIXTURE. banked_game with an HMOVE hazard planted in BANK 1.
; Purpose: the timing linter must READ bank 1. Before per-bank linting it declined the
; whole cartridge and reported nothing at all, so this trap was invisible.
; Derived from roms/techniques/banked_game.asm (see that file for the structure).
...
NextFrame:
        ...
        lda #2
        sta VBLANK
        ; The staged motion lives in bank 0 so that R1/R2 stay silent and this fixture
        ; isolates R3 alone (same no-rule-bleed discipline as the flat lint_r* fixtures).
        lda #$10
        sta HMP0
...
B1Work:                     ; レベルロード: LvTab[level*8..+7] → buf
        ; --- PLANTED TRAP: HMCLR starts 22 CPU cycles after HMOVE (<24) ---
        sta HMOVE
        ds 11, $EA          ; 11 NOP = 22 cycles
        sta HMCLR
        ldx #0
        lda level
        beq B1Copy
