; lint_bank_split — FIXTURE. banked_game with a CORRECT horizontal motion whose two
; halves live in DIFFERENT BANKS: HMP0 is staged in bank 0, HMOVE is strobed in bank 1.
; Purpose: R1/R2 ask "is HMxx/HMOVE used ANYWHERE", so a linter that surveys one bank
; at a time reports BOTH rules here and both are false. It must stay silent.
...
NextFrame:
        ...
        lda #2
        sta VBLANK
        ; --- half 1 of 2: stage the fine motion HERE, in bank 0 ---
        lda #$10
        sta HMP0
...
B1Work:                     ; レベルロード: LvTab[level*8..+7] → buf
        ; --- half 2 of 2: apply it HERE, in bank 1 (well past the 24cy window) ---
        sta WSYNC
        sta HMOVE
        ldx #0
        lda level
