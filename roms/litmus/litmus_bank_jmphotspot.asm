...
;
; 出典: 自前の検証根拠（switchEdges の分岐 C の唯一の証人）。
; SD-8b で「jmp/jsr がホットスポットへ制御を移す経路が見えていなかった＝unsound」と判定して
; 直した分岐だが、実測（2026-07-30）でコーパス123本での実行回数は **0** だった＝直したことを
; 誰も確かめていない状態だった。この ROM がその唯一の witness。
;
; 仕掛け: bank0 の可視カーネルの中で `jmp $FFF9` を実行する。$FFF9 は F8 の BANK1 ホットスポット
; なので、そこへ制御を移すと「そのアドレスの命令フェッチ自体が別バンクを選ぶ」＝この解析が
; モデル化していない遷移になる。よって領域は**名指しで拒否**されなければならない。
; 派生元: roms/techniques/banked_game.asm（構造はそちら参照）。ORIGINAL HEADER FOLLOWS.
