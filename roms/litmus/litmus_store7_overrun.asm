; litmus_store7_overrun — a kernel that `prove_line_budget` CERTIFIES and the frame rejects.
;
; This is `roms/techniques/score6.asm` with one extra load+store added to the six-store
; choreography. It exists as a negative control for the prover, not as a technique.
;
; Measured 2026-09-07:
;
;   prove_line_budget   score6  Krow worst  93 / budget 152   certified
;                       this    Krow worst 102 / budget 152   CERTIFIED
;   rendered frame      score6  262 scanlines,  8 rows of digits
;                       this    269 scanlines, 15 rows        <- broken
;
; The Krow loop must finish inside ONE line — its own comment in score6 says
; `bpl Krow ; 72 (<76 で行内完結)`. The prover gives the region a 152-cycle budget because
; it begins with `sta WSYNC` and it treats the span as two lines, so a per-line overrun of
; 8 cycles disappears into a two-line allowance. The frame-length check catches it; the
; line-budget prover alone does not. The two are not redundant.
;
...
Krow:   sta WSYNC          ; @lines 2 — last kernel line + sprite-clear cleanup span 2 visible lines; verified stable 262
        ...
        lda (p0),y          ; ★7本目: load 5cy
        sta GRP1            ; ★7本目: store 3cy
        dec row             ; 69
