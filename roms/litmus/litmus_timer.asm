...
;  B: TIM1T=$02 → 満了通過 → $93=TIMINT(満了で D7 立つ) $94=INTIM($FF ラップ域) $95=TIMINT(INTIM 読了後)
;     $96=INTIM(さらに下)。★2026-09-03 訂正: この行は $93/$94 と $95/$96 を【逆に】書いていた。
;     正本はコード :48-55（lda TIMINT→$93 / lda INTIM→$94 / lda TIMINT→$95 / lda INTIM→$96）。
;     値の側からも決まる: $94=$EF は TIMINT としては不可能（D7/D6 しか定義が無い）。下の :9-11 は
;     初めからコードと一致しており、誤っていたのはこの1行だけ。発見=蒸留（helper-3）。
