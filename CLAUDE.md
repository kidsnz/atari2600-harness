...
## リポジトリ構成（v0.11.0 再編・モノレポ）
**root = ハーネス基盤（汎用・全ゲームで再利用）**、**`roms/<game>/` = 各ロム成果物＋生成コード**に分離。
依存は **game → harness の一方通行**（harness は game にゼロ依存）。1 go.mod。
- 共通: `cmd/harness`(MCPサーバ) / `cmd/probe`(配管) / `cmd/scenario`(回帰ランナー CLI) / `internal/emu`(駆動) /
  `internal/annotate`(注釈) / `internal/scenario`(シナリオ回帰=入力タイムライン＋数値アサーション, ROM 非依存) /
  `internal/playfield/playfield.go`(汎用エンコーダ `EncodeSymmetric` 等＝Atari 2600 普遍知識)。
- 検証用ロム: `roms/litmus/`（litmus_* / smoke）。
- ロム成果物: `roms/<game>/`（`*.asm`/`*.bin`）＋ `roms/<game>/gen/`（シーン定義＋カーネル生成、`playfield` を import）
  ＋ `roms/<game>/scenarios/*.json`（回帰シナリオ）。
  例: `roms/frogger/`（Monet Frogger）＋ `roms/frogger/gen`（旧 cmd/genpf＋asmgen を統合した package main）。
- 新ゲームは `roms/<name>/`（＋`gen/`）の規約で増やす。汎用化したいカーネルは encoder 同様 root へ"昇格"（YAGNI）。
...
ROM ビルド: `dasm x.asm -f3 -ox.bin`。ロム生成: `go run ./roms/<game>/gen [scene]`。
配管検証: `go run ./cmd/probe`。MCP サーバ: `go build -o bin/harness ./cmd/harness`。
回帰シナリオ: `go run ./cmd/scenario roms/<game>/scenarios/*.json`（全 pass で exit 0）。
