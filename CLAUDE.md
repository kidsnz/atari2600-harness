...
- Base contents: `cmd/harness` (MCP server) / `cmd/probe` (plumbing) / `cmd/scenario` (regression runner CLI) /
  ...
  `cmd/framegen` (reproduction loop: FROM-SCRATCH full-frame reproduction — reads a target ROM's per-scanline object attribution (`emu.DecomposeRow`) and emits a NEW self-contained DASM source that replays PF(left/right)+GRP0/GRP1 per scanline; self-calibrates sprite X + VBLANK + content vertical-shift by assembling+rendering its own output until pixel-exact. **Reports what it did NOT reproduce** — per-element coverage in the terminal, a `; NOT REPRODUCED:` block in the generated `.asm`, and exit 1 — because its kernel emits PF+GRP0/GRP1 only: **no ENAM0/ENAM1/ENABL at all**, and one X per player for the whole frame, so missiles/ball are absent by construction and a per-zone multiplexed sprite cannot be followed. Field-measured: pixel-exact on 21/31 technique ROMs, Outlaw and Combat; partial on Fishing Derby (M1/BL absent; 96.9% overall match while P0 is 11% correct — background is 77% of the area and buries the number). See `docs/reproduce-loop.md`, audit RL-7/RL-8) /
