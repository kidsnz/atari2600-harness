...
| **the CI wall-clock LOOK-AT-IT line (20 min; nothing enforces it, `ci.yml` has no timeout. Measured 2026-08-24 over 97 runs: max 14.45 min, ZERO above 15) and the end-of-session debris sweep** — what to do when CI exceeds the ceiling, why a measurement worktree goes OUTSIDE the repo, and why the sweep must ENUMERATE the repositories rather than recite a count (it was four for two days while every handoff said three) | `docs/system-weight.md` |
