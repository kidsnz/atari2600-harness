...
| 1d. the reproduction LOOP — automated visual (`vismatch`) + behavioural (`behavmatch`) diff of a target ROM vs your build; palette-independent object-attribution + PF-table generation. Use from the START of a reproduction, not by-eye | `docs/reproduce-loop.md` |
...
- Base contents: `cmd/harness` (MCP server) / `cmd/probe` (plumbing) / `cmd/scenario` (regression runner CLI) /
  ...
  `cmd/behavmatch` (reproduction loop: behavioural diff — drives target ROM + your build through identical scripted input scenarios (`internal/behavmatch/scenarios.go`), records per-frame object trajectories, reports mechanic divergences (speed/clamp) separated from calibration (rest position) + fire→freeze coupling; `internal/behavmatch`) /
  `cmd/framegen` (reproduction loop: FROM-SCRATCH full-frame reproduction — reads a target ROM's per-scanline object attribution (`emu.DecomposeRow`) and emits a NEW self-contained DASM source that replays PF(left/right)+GRP0/GRP1 per scanline; self-calibrates sprite X + VBLANK + content vertical-shift by assembling+rendering its own output until pixel-exact. Validated: auto-generated a pixel-exact Outlaw clone incl. gunmen. See `docs/reproduce-loop.md`) /
