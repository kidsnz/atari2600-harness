...
入れると、Wayback に無いものを本家から直接取得する。Cloudflare は TLS 指紋（JA3）で
判定するため素の curl では Cookie が正しくても 403 になる → `curl_cffi`（Chrome の TLS
指紋を模倣）が入っていれば直接取得を有効化する（`pip install curl_cffi`）。AA_COOKIE が
あり curl_cffi も使える時は **本家の実ページ数を直接確認**してページ取りこぼしを防ぐ。
"""
import html
import json
import os
...
# Cloudflare を越える直接取得：AA_COOKIE + curl_cffi（Chrome の TLS 指紋模倣）。
# curl_cffi が無ければ _cffi=None → Wayback のみで従来通り動く（後方互換）。
try:
...
def direct_enabled():
    return bool(os.environ.get("AA_COOKIE")) and _cffi is not None


def direct_get(url, binary=False, timeout=60):
    """AA_COOKIE があり curl_cffi が使えれば本家 AtariAge から直接取得。
    成功時は text（binary=True なら bytes）、不可・チャレンジ・非200 なら None。"""
    cookie = os.environ.get("AA_COOKIE")
    if not cookie or _cffi is None:
        return None
    try:
        r = _cffi.get(url, headers={"Cookie": cookie.strip()},
    ...
    if not binary:
        ...
        if "just a moment" in low or "cdn-cgi/challenge" in low:
            return None  # Cloudflare チャレンジ＝Cookie 失効か指紋不一致
        return r.text
    return r.content
...
def discover_live_pages(topic_url):
    """AA_COOKIE 直接取得で本家の実ページ数を確認（Wayback の取りこぼし対策）。
    返り値: {page_no: ("LIVE", live_url)}。取得不可なら {}。"""
    body = direct_get(topic_url)
    if not body:
        return {}
    # ページ番号は **topic_id ベース**で拾う（slug がズレても外さない）。
    tid = re.search(r"/topic/(\d+)", topic_url).group(1)
    base = topic_url.rstrip("/") + "/"  # 末尾に page/N/ を足せる形（id 解決でslug不正確でも可）
    nums = {1}
    for m in re.finditer(r"/topic/" + tid + r"[\w\-]*/page/(\d+)/", body):
        nums.add(int(m.group(1)))
    m = re.search(r'data-pages="(\d+)"', body)
    if m:
        nums.add(int(m.group(1)))
    maxp = max(nums)
    return {n: ("LIVE", base if n == 1 else f"{base}page/{n}/")
            for n in range(1, maxp + 1)}


def discover_pages(topic_url):
    """全ページの取得元を列挙。CDX（Wayback・新旧ドメイン両方）の最新スナップショットに
    加え、AA_COOKIE+curl_cffi が使える時は本家の実ページ数で補完する（Wayback に無いページを
    LIVE 取得対象として追加）。返り値: {page_no: (timestamp|"LIVE", url)}。"""
    ...
    for host in (f"forums.atariage.com/topic/{slug}*",
        ...
        for row in cdx(host):
            ...
            if n not in best or ts > best[n][0]:
                best[n] = (ts, orig)
    # 本家の実ページ数で補完：Wayback に無いページ番号だけ LIVE 取得対象に足す。
    if direct_enabled():
        for n, meta in discover_live_pages(topic_url).items():
            if n not in best:
                best[n] = meta
    return best
...
def fetch_attachment(url, outdir, names, gaps):
    ...
    for row in rows:
        ...
        if code == "200" and os.path.exists(out) and os.path.getsize(out) > 200:
            return True
        time.sleep(1.5)
    blob = direct_get(url, binary=True, timeout=90)  # AA_COOKIE + curl_cffi で本家直接
...
def main():
    ...
    for n in sorted(pages):
        ...
        if not (os.path.exists(path) and os.path.getsize(path) > 10000):
            ok = False
            if ts == "LIVE":  # 本家から直接（Wayback に無いページ）
        ...
        src = "LIVE" if ts == "LIVE" else ts[:8]
        md.append(f"\n## page {n} ({len(posts)} posts, snapshot {src})\n")
        for p in posts:
            md.append(f"\n### {p['author']} — {p['time']}\n\n{p['text']}\n")
