...
| trap | note | source |
|---|---|---|
| **an ODD immediate written to a colour register** | D0 of `COLUP0`/`COLUP1`/`COLUPF`/`COLUBK` does nothing — hue is D7..D4, luminance D3..D1 — so **`#$45` is the same colour as `#$44`, and `#$46` is the next one**. Measured 2026-09-06 against the palette this repository derives from the renderer: `$44` and `$45` both give RGB(204,33,33), `$46` gives (236,51,51). Writing an odd value never crashes, which is why nothing caught it; it is a sign the author believes in a colour that is not there. **The machine has 128 colours, not 256**, and an odd literal rounds DOWN — never towards the one the author was probably reaching for. `cmd/palette` names its swatches with the even code for the same reason, and a 256-entry palette file shows 128 colours the machine cannot make. | static: `scripts/check_traps.py` warns on `lda #$xx` (odd) immediately followed by `sta COLUxx`. Positive and negative controls both run; **zero hits across the tree's 411 files**, so it is a guard against new code rather than a backlog | Found by the mailing-list distillation (helper-3), who checked first — `rg -ni 'odd colour\|odd color' docs/ pkg/design/` returned nothing — and measured here |
| **an object drawn only as SCENERY still sets its collision latch** | The TIA compares geometry and has no notion of what a program MEANS by an object. Borrow a missile for an explosion, a shadow or a piece of the background and it keeps colliding at its first job — the latch is set by an overlap the program is no longer thinking about. This repository teaches the borrowing (`design-principles.md`: *"Multi-kernel = reuse one object per region"*, and the list's *"intelligent sprite flicker and reuse"*); until 2026-09-06 it did not teach the bill. **Measured** (`roms/litmus/litmus_decor_collides.asm`, `internal/emu/decorcollide_test.go`): a missile enabled inside a quad-width player reads `CXM1P` = **$81, D7 set**; clear `ENAM1`, leave everything else identical, and it reads **$01, D7 clear**. The fix is not `CXCLR` after the fact — the latch is sticky, so clearing it only moves the problem to whoever reads next; stop TESTING the collision for the frames the object is on loan. ★And a note on measuring it: the first fixture strobed `RESP0` and `RESM1` three CPU cycles apart and read no collision at all, which looks exactly like the trap being absent. `DecomposeRow` put the objects at clocks 3..10 and 25..32 — twenty-two apart, never touching. **A negative result from a fixture that never posed the question is not a negative result**, so the test now asserts the overlap before it asserts anything about the latch. | The bug report is from the author of the game it happened in: Piero Cavina on INV, *"This must happen because the program is checking missile/invaders collisions even when the missile is used for the explosion"* 〔`199801/msg00197`, 1998-01〕 — the symptom was that shooting one of two adjacent invaders in the top row exploded both. Found by the mailing-list distillation (helper-1), measured here |
