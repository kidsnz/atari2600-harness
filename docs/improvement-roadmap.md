...
| B タイミング | サイクル計算が合わない | **半閉** | 位置は閉。**サイクル予算の検証が未着手（P0・本丸）** |
...
### B-3. per-scanline サイクル予算ガード（本丸 — Pong v2 を殺した失敗を直接検知）
...
### B-4. kernel 依存定数の自動キャリブレーション（欠落C と接続）
