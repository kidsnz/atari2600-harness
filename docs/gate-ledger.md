...
| `check_wiring.py` | an orphaned doc, a litmus ROM nothing runs, ★a scenario check no document names, ★a `pkg/design` rule no document names, a `verified-coverage.md` row whose ROM is absent or unrun, a gate with no ledger row, **an internal package nothing imports, a command CLAUDE.md never names** | `ci` + `pre-push` | 1.57 s | **3** | many | 1 |
