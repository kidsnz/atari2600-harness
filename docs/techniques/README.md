...
| 36 | RESPx RE-STROBING — how many shaped slots fit on ONE scanline. A player in a copy mode draws **3 + k** copies with k mid-line strobes, so one player reaches **eight** and two reach sixteen, and the six-slot figure everyone quotes is the k=0 rung. Each strobe costs the copy at its own base (technique 35 rule 8) plus the one still pending from the old base, and buys the two the new base makes. **An added copy is never at x ≡ 0 (mod 3)**, and the leading two copies cannot be moved at all. Slots are not the binding limit though — BYTES are, at six cycles a write | advanced | [restrobe-copies.md](restrobe-copies.md) | `roms/litmus/litmus_restrobe.asm` | ✅ ladder 3..8 measured over 6 bands, CI-locked; source solidcorp AtariAge 180632 |
