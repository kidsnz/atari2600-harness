...
- **VV-10 (HW-trap detectors):** siblings of `assert_line_budget` in `internal/emu`. **T-1 ✅ DONE (v1.85.0):**
  RIOT timer-wrap (=G8). ⚠ **Why a WATCHER and not an interrupt handler, stated here because it is the reason this whole detector has to exist:** the RIOT can raise a timer interrupt and **nothing on the 6507 can hear it**. Eckhard Stolberg, 2002: *"The 6507 processor has the **interrupt pin removed to save costs** … The timer could generate an interrupt, but the **processor won't be able to sense it**. **You'll have to poll the timer**, I'm afraid"*; Roger Williams, 2001: *"The 6507 **does not bring the IRQ line to the outside world**"*; Frank Palazzolo, 1997, the same split five years earlier. So overrunning the timer is not an event a program can be told about — it can only be **found by looking**, which is what this does, and why `missiles-bullets.md`'s finding that `SEI` does nothing here is the same fact seen from the other side. ★Eric Ball adds the reason it would be unaffordable even if the pin existed: *"an interrupt automatically **pushes the PC & PSR on the stack** and you only have **128 bytes of RAM**"* — three of them per interrupt, against the four a whole kernel typically spends 〔see the stack budget in `design-principles.md`〕. `Emu.TimerState` exposes the timer (INTIM/TIMINT/Expired/Divider/ticks); `Emu.WatchTimerWrap`
