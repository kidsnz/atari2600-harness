...
- **VV-2 ✅ DONE (v1.80.0):** `internal/cyclebound`+`cmd/cyclebound` recursive-descent-decode the ROM from its
  ...
  **`prove_line_budget` MCP tool**, and scenario **`checks.prove_line_budget`** (`cyclebound_safe.json` certifies
  ...
  violation (non-vacuous); the certified bound holds at runtime (observed-within-proven dual). Scope: single-bank
  flat 2K/4K. **Src:** Li&Malik IPET DAC'95; Ballabriga&Cassé WCET'08.
