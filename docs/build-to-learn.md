...
3. **★寸法スペックを先に確定（Phase 0 で必須・後回し厳禁）**（`_casestudies/<game>/layout-compare.ja.md`）：`get_screen_annotated`（TIA実座標で校正された「目」）で**オリジナルの全要素の位置・サイズ(Y/clock/幅/走査線数)を最初に測り切り、目標スペックにする**。各段はこのスペックに合わせて作る。**理由＝レイアウトはカーネル区画・走査線配分・位置に直結し、制作が進んでから直すと全やり直しになる**（＝色/RAMと同じく「測ってから作る」）。自作ができたら annotated で差分確認・収束（read_row/annotated）。〔ユーザー指摘 2026-06-15：レイアウト測定は一番最初に〕
